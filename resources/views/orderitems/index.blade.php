...
<div class="d-flex justify-content-between align-items-center mb-3">
  ...
  <a href="{{ route('orderitems.create',$order) }}" class="btn btn-primary">{{ __('messages.orderitems.add') }}</a>
</div>
<table class="table table-striped">
  ...
  @foreach($orderitems as $orderitem)
    <tr>
      ...
      <td class="text-end">
        <div class="btn-group">
            <a href="{{ route('orderitems.edit', [$order, $orderitem]) }}" class="btn btn-warning btn-sm">
                <i class="fas fa-edit"></i>
            </a>
            <form action="{{ route('orderitems.destroy', [$order, $orderitem]) }}" method="POST" class="d-inline">
                @csrf
                @method('DELETE')
